...
if response.status_code == 200:
    ...
    for visualization in visualizations_data:
        ...
        print(f"Visualization Name: {visualization_name}, Visualization ID: {visualization_id}")
        ...
        response = requests.get(visualization_data_url, headers={'Authorization': f'Bearer {access_token}'})
        ...
        if response.status_code == 200:
            ...
            df.to_csv(f'power_bi_visualization_data_{visualization_name}.csv', index=False)
            print(f"Visualization data for {visualization_name} exported to CSV successfully.")
        else:
            print(f"Error retrieving visualization data. Status code: {response.status_code}, Response: {response.text}")
else:
    print(f"Error retrieving visualizations. Status code: {response.status_code}, Response: {response.text}")
