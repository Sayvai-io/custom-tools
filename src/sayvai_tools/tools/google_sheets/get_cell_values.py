...
class GetCellValuesTool(GoogleSheetsBaseTool):
    ...
    description: str = "Use this tool to retrieve cell values from a specified range in a Google Sheets spreadsheet."
    args_schema: Type[GetCellValuesSchema] = GetCellValuesSchema

    def _get_cell_values(
        self,
        spreadsheet_id: str,
        range_: str
    ) -> List[List[str]]:
        ...
        """
        try:
            result = self.api_resource.spreadsheets().values().get(
